module ClassicColorConversionsHelper
  ...
  def check_rgb_converted_to_color(colour_space, fixture_color, expected_rgb)
    if [:name, :hex].include?(colour_space)
      converted_color = ColorConverters::Color.new(expected_rgb).send(colour_space)
  ...
  def check_colour_converted_to_rgb(colour_space, fixture_color, expected_rgb)
    if [:name, :hex].include?(colour_space)
