RSpec.shared_examples 'classic_colour_conversions' do
  ...
  describe 'two-way conversions' do
    it 'converts black and back' do
      ...
      check_rgb_converted_to_color(colour_space, passed_color, expected_rgb)
      check_colour_converted_to_rgb(colour_space, passed_color, expected_rgb)
    end
...
RSpec.shared_examples 'custom_colour_conversions' do
  ...
  #       ColorConverters::Color.new(**expected_rgb).send(colour_space).each { |key, value| expect(passed_color[key]).to be_within(rounding_margin).of value }
