...
RSpec.describe ColorConverters::CielchConverter do
  context 'methods' do
    ...
    it '.input_to_rgba' do
      expect(described_class.new(l: 45.03, c: 60.80, h: 287.92, space: :cie).rgba).to eq({ r: 50.99, g: 102.0, b: 203.99, a: 1.0 })
      expect(described_class.new(l: '45.03', c: '60.80', h: '287.92', space: 'cie').rgba).to eq({ r: 50.99, g: 102.0, b: 203.99, a: 1.0 })

      expect(described_class.new(l: 33.673, c: 15.512, h: 47.284, space: :cie).rgba).to eq({ r: 101.25, g: 72.70, b: 61.67, a: 1.0 })
      expect(described_class.new(l: '33.673', c: '15.512', h: '47.284', space: 'cie').rgba).to eq({ r: 101.25, g: 72.70, b: 61.67, a: 1.0 })
    end

    it '.input_to_rgba and back' do
      cielch = { l: 73.53, c: 33.54, h: 36.31 }
      ...
      cielch = { l: 45.02, c: 60.80, h: 287.92 }
