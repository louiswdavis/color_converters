...
RSpec.describe ColorConversion::OklchConverter do
  context 'methods' do
    it '.matches?' do
      expect(described_class.matches?(l: 74, c: 35, h: 37)).to be true
      expect(described_class.matches?(l: 74, c: 35, z: 37)).to be false
      expect(described_class.matches?('#ffffff')).to be false
    end

    it '.validate_input' do
      expect { described_class.new(l: 174, c: 35, h: 37) }.to raise_error(ColorConversion::InvalidColorError)
      expect { described_class.new(l: 74, c: 135, h: 37) }.to raise_error(ColorConversion::InvalidColorError)
      expect { described_class.new(l: 74, c: 35, h: 437) }.to raise_error(ColorConversion::InvalidColorError)
    end

    it '.input_to_rgba' do
      expect(described_class.new(l: 45.03, c: 60.80, h: 287.92).rgba).to eq({ r: 50.99, g: 102.0, b: 203.99, a: 1.0 })
      expect(described_class.new(l: '45.03', c: '60.80', h: '287.92').rgba).to eq({ r: 50.99, g: 102.0, b: 203.99, a: 1.0 })

      expect(described_class.new(l: 33.673, c: 15.512, h: 47.284).rgba).to eq({ r: 101.25, g: 72.70, b: 61.67, a: 1.0 })
      expect(described_class.new(l: '33.673', c: '15.512', h: '47.284').rgba).to eq({ r: 101.25, g: 72.70, b: 61.67, a: 1.0 })
    end
